...
func (e *OpenAICompatExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if repairedPayload, changed := repairOpenAICompatInvalidJSONStringEscapes(req.Payload); changed {
	...
	if len(opts.OriginalRequest) > 0 {
		if repairedOriginal, changed := repairOpenAICompatInvalidJSONStringEscapes(opts.OriginalRequest); changed {
...
func repairOpenAICompatInvalidJSONStringEscapes(payload []byte) ([]byte, bool) {
	if json.Valid(payload) {
		return payload, false
	}
	repaired, changed := repairInvalidJSONStringEscapes(payload)
